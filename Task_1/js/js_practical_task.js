...
function secondsToDate(seconds) {
    var startTime = new Date(2020,6,1,0,0,0.00202);
    var milliseconds = startTime.getTime() + (seconds * 1000);
...
function repeatingLitters(string) {
    ...
    for(let i = 0; i < string.length; i++){
        ...
        if(substringOccurrencesCounter(string.charAt(i),string) == 1){
            result += string.charAt(i);
        }
    }
    return result;
 ...
 */
function gather(...str) {
    return function order(...matrix){
        let resultString = "";
        for(const matrixElement of matrix){
            resultString += str[matrixElement];
        }
        return resultString;
